...
body.overview .slide-inner{position:absolute;top:50%;left:50%;width:100vw;height:100vh;transform-origin:center;overflow:hidden;pointer-events:none;box-sizing:border-box}
...
function applyOverviewScale(){if(!document.body.classList.contains('overview'))return;const cellW=slides[0].getBoundingClientRect().width,scale=Math.max(cellW/window.innerWidth,(cellW*9/16)/window.innerHeight);slides.forEach(s=>{const inner=s.querySelector(':scope>.slide-inner');if(inner)inner.style.transform='translate(-50%,-50%) scale('+scale+')';});}
...
document.addEventListener('scroll',showCtrl,{passive:true,capture:true});
showCtrl();
