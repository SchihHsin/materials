...
function applyOverviewScale(){if(!document.body.classList.contains('overview'))return;const scale=slides[0].getBoundingClientRect().width/window.innerWidth;slides.forEach(s=>{const inner=s.querySelector(':scope>.slide-inner');if(inner)inner.style.transform='scale('+scale+')';});}
