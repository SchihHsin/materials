...
## 页面密度路由（2026-07，已验证，写 skill 用）
- **先分阅读意图，不先数文字**：灰底分析页走 D 系列；灰底但以截图、现场材料、关系图为主要分析证据时走 A 系列；黑底方案、界面或路径需要被“感受到”时走 B 系列。A / B 不与 D 争夺档位。
- **D1--D4 = 灰底分析密度**：D1（低密度）= 1 主张 + 2--3 个支撑，结论最大；D2（中低）= 1 判断 + 1 组证据；D3（中高）= 1 判断 + 3 个信号 + 4 个动作，按结构收束；D4（高）= 多维对照 / 查阅页，必须表格、分组或固定字段。默认 D1；密度上升优先增加结构，不缩普通正文。主叙事页通常到 D3，D4 仅在确有查阅需求或用户明确要求高密度时使用；内容再多仍可做高密度页，但先选模板并保证结构清楚。
- **A1--A3 = 灰底分析视觉页**：A1 一张关键证据图 + 判断 + 2--3 条定位观察；A2 2--3 张材料并置 + 差异结论；A3 一张关系图 / 路径图 + 结构化解读。图必须是分析证据，不能只是装饰背景。
- **B1--B3 = 黑底设计叙事页**：B1 大图 + 一个设计主张；B2 界面 / 截图 + 2--3 个标注；B3 路径 / 机制图 + 节点解释。它们由图像构图决定，不按 D1--D4 缩放文字。
...
- **渐变 token**（`:root`）：分类/状态 `--g-red/green/blue/amber/purple/teal/pink`，蓝紫 `--g-accent`，**统一深色** `--g-ink`（渐变 #22262F→#14171C）、`--dark`（实色 #16191E），中性 `--g-neutral`（浅灰渐变）
- **大圆角 + 渐变**：图表色块普遍带渐变；**绿/红等状态色用同明度「微色相位移」渐变**（如绿 `#23CFA0→#45D65A`、红 `#FF5A6E→#FF6F55`，不做深浅明暗），**中性用浅灰渐变**
...
- **④ 用户旅程**：CANN 专属阶段（环境搭建/文档学习/算子开发/调试优化/集成发布）；6 行 = 阶段 / 触点(中性色) / 行为(mini UI 线框截图) / **情绪曲线(5 个独立格子，每格曲线段+渐变填充+虚线横纹，`flex-shrink:0` 锁高；格间可留白，但每条线在相邻格公共边界的高度必须严格一致；多条线各自连续，彼此无需同高)** / 痛点(每列 2 条) / 机会点(每列 2 条)
